...
export const CommandsLayout: React.FC<ICommandsLayoutProps> = ({
	...
	const {
		state: { commands, rover },
		dispatch,
	} = useContext(AppContext);

	const handleChangeInput = useCallback(
		command => {
			if (COMMAND_LIST.includes(command.slice(-1))) {
				// The last key input is in the command list
				// Update the input
				dispatch({
					type: actions.UPDATE_COMMANDS,
					payload: command,
				});

				// Move the Rover
				const { newDir, newPos } = moveRover(
					command.slice(-1) as RoverCommands,
					rover.direction,
					rover.position,
				);
				dispatch({
					type: actions.UPDATE_ROVER_DIRECTION,
					payload: newDir,
				});
				dispatch({
					type: actions.UPDATE_ROVER_POSITION,
					payload: newPos,
				});
			}
		},
		...
		>
			...
			<div className="CommandsLayout-help">
				...
				<dl className="CommandsLayout-command-list">
					...
					<dd className="CommandsLayout-desc">
						Move forward
					</dd>
